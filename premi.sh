...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To NovanThekilz ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e " This Will Quick Setup VPN Server On Your Server"
echo -e "  Auther : ${green}kilzz® ${NC}${YELLOW}(${NC} ${green} NovanThekillz${NC}${YELLOW})${NC}"
echo -e " © Recode By Thekilz${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 2
###### IZIN SC 
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/Bezzo99/mau/kepo/yah | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/Bezzo99/mau/kepo/yah | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/Bezzo99/mau/kepo/yah | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/Bezzo99/tes3/main/"
...
echo -e " ${BLUE}        └────────────────────────────────────┘${NC}"
echo -e "${L1}          Pilih Sesuai OS Anda ${GREEN}$(cat /etc/os-release | grep -w PRETTY_NAME | head -n1 | sed 's/=//g' | sed 's/"//g' | sed 's/PRETTY_NAME//g')${NC}"
echo -e " ${BLUE}        ┌────────────────────────────────────┐${NC}"
echo -e " ${BLUE}        │${NC}[${GREEN}01${NC}]${L1} INSTALLER UBUNTU ${NC}( ${GREEN}Focal (20.04 LTS), Jammy (22.04 LTS), Noble (24.04 LTS) ${NC} )${NC}"
echo -e " ${BLUE}        │${NC}[${GREEN}02${NC}]${L1} INSTALLER DEBIAN ${NC}( ${GREEN}Buster (10), Bullseye (11), dan Bookworm (12) ${NC} )${NC}"
echo -e " ${BLUE}        └────────────────────────────────────┘${NC}"
echo -e ""
...
case $menu in
    1 | 01)
        # Ubuntu: Focal (20.04 LTS), Jammy (22.04 LTS), Noble (24.04 LTS)
        clear
        # Mendapatkan nama kode distribusi Ubuntu
        ...
        if [ "$DISTRO_CODENAME" == "noble" ]; then
            echo -e "${YELLOW}Deteksi: Ubuntu Noble (24.04 LTS). Menginstal HAProxy dari repositori resmi.${NC}"
            # Untuk Noble, gunakan repositori resmi karena PPA vbernat/haproxy-2.8 belum mendukungnya.
            # Versi yang diinstal akan tergantung pada apa yang tersedia di repositori Noble.
            apt-get update -y > /dev/null 2>&1
            apt-get -y install haproxy > /dev/null 2>&1
            apt-get -y install socat > /dev/null 2>&1
        else
            echo -e "${YELLOW}Deteksi: Ubuntu ${DISTRO_CODENAME}. Menginstal HAProxy dari PPA vbernat/haproxy-2.8.${NC}"
            # Untuk Focal dan Jammy, gunakan PPA vbernat/haproxy-2.8
    ...
    ;;
    2 | 02)
        # Debian: Buster (10), Bullseye (11), dan Bookworm (12)
        clear
        ...
        # Mendeteksi versi Debian
        DEBIAN_VERSION=$(grep VERSION_ID /etc/os-release | cut -d= -f2 | tr -d '"')

        if [ "$DEBIAN_VERSION" == "10" ]; then
            # Debian 10 (Buster) - Menggunakan backports untuk HAProxy 2.4
            ...
            # Debian 11 (Bullseye) - Menggunakan backports untuk HAProxy 2.8
            wget -O - https://haproxy.debian.net/bernat.debian.org.gpg | apt-key add - > /dev/null 2>&1
            ...
            # Debian 12 (Bookworm) - Menggunakan repositori backports standar untuk HAProxy 2.8
...
function nginx_install() {
    ...
    if [[ "${OS_ID}" == "ubuntu" ]]; then
        echo -e "${BLUE}-----------------------------------------------------${NC}"
        echo -e "${BLUE}  Memulai instalasi Nginx untuk Ubuntu (${OS_PRETTY_NAME})${NC}"
        echo -e "${BLUE}-----------------------------------------------------${NC}"
        sudo apt update -y
        ...
        if [ $? -eq 0 ]; then
            ...
            echo -e "${RED_BOLD}-----------------------------------------------------${NC}"
            echo -e "${RED_BOLD}  Gagal menginstal Nginx pada Ubuntu (${OS_PRETTY_NAME}).${NC}"
            echo -e "${RED_BOLD}  Silakan periksa koneksi internet atau repository Anda.${NC}"
            echo -e "${RED_BOLD}-----------------------------------------------------${NC}"
        fi
    elif [[ "${OS_ID}" == "debian" ]]; then
        echo -e "${BLUE}-----------------------------------------------------${NC}"
        echo -e "${BLUE}  Memulai instalasi Nginx untuk Debian (${OS_PRETTY_NAME})${NC}"
        echo -e "${BLUE}-----------------------------------------------------${NC}"
        sudo apt update -y
        sudo apt install nginx -y
        if [ $? -eq 0 ]; then
            echo -e "${GREEN}-----------------------------------------------------${NC}"
            echo -e "${GREEN}  Nginx berhasil diinstal pada Debian (${OS_PRETTY_NAME}).${NC}"
            echo -e "${GREEN}-----------------------------------------------------${NC}"
            /etc/init.d/nginx restart
            /etc/init.d/nginx status
        else
            echo -e "${RED_BOLD}-----------------------------------------------------${NC}"
            echo -e "${RED_BOLD}  Gagal menginstal Nginx pada Debian (${OS_PRETTY_NAME}).${NC}"
            echo -e "${RED_BOLD}  Silakan periksa koneksi internet atau repository Anda.${NC}"
            echo -e "${RED_BOLD}-----------------------------------------------------${NC}"
        fi
    else
        echo -e "${YELLOW_BOLD}-----------------------------------------------------${NC}"
        echo -e "${YELLOW_BOLD}  OS Anda (${OS_PRETTY_NAME}) saat ini tidak didukung untuk instalasi Nginx ini.${NC}"
        echo -e "${YELLOW_BOLD}  Fungsi ini hanya mendukung Ubuntu dan Debian.${NC}"
        echo -e "${YELLOW_BOLD}-----------------------------------------------------${NC}"
    fi
}
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/Bezzo99/mau/kepo/yah | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/Bezzo99/mau/kepo/yah | grep $ipsaya | awk '{print $3}')
...
clear
# banner
echo "Banner /etc/novan.txt" >>/etc/ssh/sshd_config
sed -i 's@DROPBEAR_BANNER=""@DROPBEAR_BANNER="/etc/novan.txt"@g' /etc/default/dropbear

# Ganti Banner
wget -O /etc/novan.txt "${REPO}limit/issue.net"
